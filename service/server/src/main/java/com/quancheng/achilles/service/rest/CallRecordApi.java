...
@Api(value = "call-record", description = "the call-record API")
public interface CallRecordApi {

	@ApiOperation(value = "", notes = "数据中心集市表 》客户服务 》 通话记录", response = CallRecord.class, responseContainer = "List", tags = {
	        "客服服务", })
	@ApiResponses(value = { @ApiResponse(code = 200, message = "订单列表", response = CallRecord.class) })
	@RequestMapping(value = "/call-record/list", produces = { "application/json" }, method = RequestMethod.GET)
	ResponseEntity<List<CallRecord>> callRecordListGet(
